...
import { isFieldDuplicated, storeForms, getValues, preprocess_and_validate_field } from '../utils';
...
storeForms.subscribe(async (forms) => {
    let form = forms.find((form) => form.form_name === form_name);
    if (form) {
        const _fields = await Promise.all(form.fields.map(async (field) => {
            // Preprocess and validate field.
            field = await preprocess_and_validate_field(form, field, values);
            return field;
        }));
        const dirty = fields.find((field) => {
            if (field.validation) {
...
{#if current_form}
	...
	{:else}
		<form bind:this={_formEl} on:submit|preventDefault={onSubmit} on:reset|preventDefault={onReset}>
			...
			<button type="reset" class="outline btn-primary">Reset</button>
		</form>
	{/if}
{/if}
